...
async function main() {
  ...
  const propRepo  = ds.getRepository(Property);
  const revRepo   = ds.getRepository(Review);
  ...
  for (const p of PERSONAS) {
    ...
    if (!u) {
      u = userRepo.create({
        ...p,
        password: await bcrypt.hash("Asavio2025!", 10),
        role: "user",
        isVerified: true,
  ...
  for (const host of hosts) {
    ...
    for (const prop of properties) {
      ...
      const dateOffsets = Array.from({ length: count }, (_, i) =>
        rand(i * Math.floor(60 / count) + 1, (i + 1) * Math.floor(60 / count))
      ...
      for (let i = 0; i < count; i++) {
        ...
        (rev as any).createdAt = createdAt;
        (rev as any).updatedAt = createdAt;
      ...
      for (let i = 0; i < CONV_COUNT; i++) {
        ...
        const conv = convRepo.create({
          ...
          hostFirstReplyAt:   i < REPLIED_COUNT
            ? new Date(guestMsgAt.getTime() + rand(30, 180) * 60_000) // replied in 30–180 min
            : null,
    ...
    const allReviews = await ds.getRepository(Review)
