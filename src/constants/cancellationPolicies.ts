...
export const CANCELLATION_POLICIES = {
  flexible: {
    ...
    description:
      "Guests can cancel for a full refund (excluding platform fee) up to 24 hours before check-in. " +
  ...
  moderate: {
    ...
    description:
      "Guests can cancel for a full refund (excluding platform fee) up to 5 days before check-in. " +
  ...
  firm: {
    ...
    description:
      "Full refund (excluding platform fee) if cancelled 14+ days before check-in. " +
  ...
  strict: {
    ...
    description:
      "Full refund (excluding platform fee) if cancelled 30+ days before check-in. " +
...
export const POLICY_TYPES = Object.keys(CANCELLATION_POLICIES) as CancellationPolicyType[];
 ...
 */
export function calculateRefund(opts: {
  totalPrice: number;
  hostPayout: number;
  checkIn: Date;
  createdAt: Date;
  ...
  const policy = (opts.policy as CancellationPolicyType) in CANCELLATION_POLICIES
    ? (opts.policy as CancellationPolicyType)
    : "flexible";
  ...
  // Grace period check
  const inGracePeriod =
    hoursSinceBooking <= GRACE_PERIOD_HOURS && daysUntilCheckIn >= GRACE_MIN_DAYS_AHEAD;
