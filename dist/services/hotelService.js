...
class HotelService {
    get hotelRepo() { return database_1.AppDataSource.getRepository(Hotel_1.Hotel); }
    get roomRepo() { return database_1.AppDataSource.getRepository(RoomType_1.RoomType); }
    get hotelImgRepo() { return database_1.AppDataSource.getRepository(HotelImage_1.HotelImage); }
    get roomImgRepo() { return database_1.AppDataSource.getRepository(RoomTypeImage_1.RoomTypeImage); }
    get bookingRepo() { return database_1.AppDataSource.getRepository(Booking_1.Booking); }
    // ── Create ────────────────────────────────────────────────────────────
    async createHotel(hostId, input, files) {
        // Tier limit check
        await subscriptionService_1.subscriptionService.checkListingLimit(hostId, "hotel");
        ...
        if (files && files.length > 0) {
            for (let i = 0; i < files.length; i++) {
                ...
                await this.hotelImgRepo.save(this.hotelImgRepo.create({
                    hotelId: saved.id,
                    url: result.url,
    ...
    async getHotels(filters = {}) {
        const { city, hotelType, star, minPrice, maxPrice, guests, sort = "newest", page = 1, limit = 12, } = filters;
        ...
        if (hotelType)
            qb.andWhere("h.hotelType = :hotelType", { hotelType });
        if (star !== undefined)
            qb.andWhere("h.starRating >= :star", { star });
        // Price filters go against the cheapest room type per hotel
        if (minPrice !== undefined || maxPrice !== undefined || guests !== undefined) {
            const sub = database_1.AppDataSource.getRepository(RoomType_1.RoomType)
            ...
            qb.andWhere((qbInner) => {
                ...
                if (minPrice !== undefined)
                    subQ.andWhere("rt.pricePerNight >= :minPrice", { minPrice });
                if (maxPrice !== undefined)
                    subQ.andWhere("rt.pricePerNight <= :maxPrice", { maxPrice });
                if (guests !== undefined)
                    subQ.andWhere("rt.maxGuests >= :guests", { guests });
     ...
     */
    async getHotelTypeRepresentatives() {
    ...
    async getAvailableHotelTypes() {
        const rows = await database_1.AppDataSource.getRepository(Hotel_1.Hotel)
    ...
    // ── Update ────────────────────────────────────────────────────────────
    async updateHotel(id, hostId, role, updates, files) {
        const hotel = await this.getHotelById(id);
        if (role !== "admin" && hotel.hostId !== hostId) {
            throw new AppError_1.AppError("Not authorised to update this hotel", 403);
        }
        ...
        if ("starRating" in cleanUpdates) {
            ...
            if (role !== "admin")
                cleanUpdates.verifiedStarRating = false;
        }
    ...
    // ── Delete ────────────────────────────────────────────────────────────
    async deleteHotel(id, hostId, role) {
        const hotel = await this.getHotelById(id);
        if (role !== "admin" && hotel.hostId !== hostId) {
            throw new AppError_1.AppError("Not authorised to delete this hotel", 403);
        }
    ...
    // ── Room types ────────────────────────────────────────────────────────
    async createRoomType(hotelId, hostId, role, input, files) {
        ...
        const effectiveTier = (hotel.host?.subscriptionTier ?? "starter");
        const limit = tierConfig[effectiveTier].maxRoomTypes;
        if (limit !== Infinity) {
            const current = await this.roomRepo.count({ where: { hotelId } });
            if (current >= limit) {
                throw new AppError_1.AppError(`Your ${tierConfig[effectiveTier].label} plan allows up to ${limit} room types per hotel. Upgrade to add more.`, 403);
            }
        }
        ...
        if (files && files.length > 0) {
            for (let i = 0; i < files.length; i++) {
                ...
                await this.roomImgRepo.save(this.roomImgRepo.create({
                    roomTypeId: saved.id,
                    url: result.url,
                    publicId: result.publicId,
                    isPrimary: i === 0,
    ...
    async getRoomTypeById(id) {
        ...
        if (!room)
            throw new AppError_1.AppError("Room type not found", 404);
        return room;
    }
    async updateRoomType(roomId, hostId, role, updates, files) {
        ...
        if ("pricePerNight" in cleanUpdates)
            cleanUpdates.pricePerNight = Number(cleanUpdates.pricePerNight);
        if ("maxGuests" in cleanUpdates)
            cleanUpdates.maxGuests = Number(cleanUpdates.maxGuests);
        if ("totalUnits" in cleanUpdates)
            cleanUpdates.totalUnits = Number(cleanUpdates.totalUnits);
        Object.assign(room, cleanUpdates);
        await this.roomRepo.save(room);
        return this.getRoomTypeById(roomId);
    }
    async deleteRoomType(roomId, hostId, role) {
        const room = await this.getRoomTypeById(roomId);
        if (role !== "admin" && room.hotel?.hostId !== hostId) {
            throw new AppError_1.AppError("Not authorised", 403);
        }
     ...
     */
    async getRoomAvailability(hotelId, checkIn, checkOut) {
        const hotel = await this.getHotelById(hotelId);
